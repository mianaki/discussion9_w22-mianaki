...
def get_valid_emails(prof_list):
    ...
    Examples of invalid email addresses:
        ...
        vgvinodv@umich_.edu
    """
    reg_exp = r'\b[a-z]+@umich.edu$'
    valid_profs = {}
    for prof in prof_list:
        x = re.findall(reg_exp, prof[-1])
        if x:
            valid_profs[prof[0]] = prof[-1]
    return valid_profs
...
def get_lsa_majors():
    ...
    and the Arts.
    """
    resp = requests.get('https://admissions.umich.edu/academics-majors/majors-degrees')
    soup = BeautifulSoup(resp.content, 'html.parser')
    tags = soup.find_all('td', class_= 'views-field w-7/12 hover:text-tappan-red views-field-title')
    majors = []
    for tag in tags:
        link = tag.find('a').get('href')
        if link:
            if 'lsa' in link:
                majors.append(tag.text.strip())
